...
Even if coexistence fixed points *can* exist algebraically, they must be biological (i.e., $x^* < K$). A transcritical bifurcation point occurs when a coexistence fixed point collides with the prey-only fixed point $(K,0)$. At this point, $x^* = K$.
