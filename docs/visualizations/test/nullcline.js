...
// ----------------------------
function FE(x, alpha) {
  return x * Math.exp(-alpha * x);
}

// Find roots of x*exp(-alpha*x) = gamma
function solveGammaRoots(alpha, gamma) {
  if (alpha === 0) return [gamma]; // Simple x = gamma
  const roots = [];
  const xMax = 1 / alpha;
  const maxVal = FE(xMax, alpha);
  
  if (maxVal < gamma) return roots; // No roots exist
  
  // Left root
  let left = 0, right = xMax;
  for (let i = 0; i < 50; i++) {
    let mid = (left + right) / 2;
    if (FE(mid, alpha) < gamma) left = mid; else right = mid;
  }
  roots.push((left + right) / 2);
  
  // Right root
  if (maxVal > gamma) {
    let l2 = xMax, r2 = Math.max(xMax * 10, 100);
    for (let i = 0; i < 50; i++) {
      let mid = (l2 + r2) / 2;
      if (FE(mid, alpha) > gamma) l2 = mid; else r2 = mid;
    }
    roots.push((l2 + r2) / 2);
  }
  return roots;
...
// Plotting
// ----------------------------
function drawPhasePlane(svgId, r, K, alpha, options) {
  const gamma = 1.0;
  ...
  for (let x = xMin; x <= xMax + 0.05; x += 0.05) {
      let y = r * (1 - x / K) * Math.exp(alpha * x);
      xNullData.push([x, y]);
  }
  ...
  const yRoots = solveGammaRoots(alpha, gamma);
  yRoots.forEach(root => {
      g.append("line")
          .attr("x1", xScale(root))
          .attr("x2", xScale(root))
  ...
  if (options?.showSim) {
      ...
      for (let t = 0; t < Ttotal; t += dt) {
          const fx = (X, Y) => r * X * (1 - X / K) - FE(X, alpha) * Y;
          const fy = (X, Y) => Y * (FE(X, alpha) - gamma);
...
function updatePlots() {
  ...
  drawPhasePlane("#plot1", options.r, options.k1, options.alpha1, options);
  drawPhasePlane("#plot2", options.r, options.k2, options.alpha2, options);
}

["r", "alpha1", "alpha2", "k1", "k2", "x0", "y0", "propLength", "showSim"].forEach(id => {
