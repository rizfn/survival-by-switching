...
class Particle {
  ...
  update(dt) {
    ...
    // Double simulation speed (dt * 1.0)
    const simDt = dt * 2.0;
    this.x += dx * simDt;
    this.y += dy * simDt;
  }
  ...
  draw() {
    if (this.y < 0 && this.prevY < 0) return; // Do not render if inadvertently negative (should not happen mathematically if y>0 initially)

    const pos = toScreen(this.x, this.y);
    const prevPos = toScreen(this.prevX, this.prevY);
...
function initParticles() {
  ...
  for (let i = 0; i <= stepsX; i++) {
    for (let j = 0; j <= stepsY; j++) {
      const pX = xMin + (xMax - xMin) * (i / stepsX);
      const pY = 0.01 + (yMax) * (j / stepsY);
      if (pY >= 0 && pY <= yMax) {
...
function updateEnvText() {
    isAverageMode = (switchIntervalSec === 0);
    if (isAverageMode) {
        envText.innerText = `Infinite Switching (Averaged Env)`;
    } else {
        envText.innerText = `Switching Environment every ${switchIntervalSec.toFixed(2)}s`;
    }
...
document.addEventListener('keydown', (e) => {
  if (e.code === 'Space') {
    ...
    e.preventDefault();
    switchIntervalSec = Math.min(2.0, switchIntervalSec + 0.01);
    updateEnvText();
  } else if (e.code === 'ArrowDown' || e.key === 's' || e.key === 'S') {
    e.preventDefault();
    switchIntervalSec = Math.max(0.0, switchIntervalSec - 0.01);
